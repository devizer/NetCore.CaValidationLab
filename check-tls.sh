#!/usr/bin/env bash
# one liner
# url=https://raw.githubusercontent.com/devizer/NetCore.CaValidationLab/master/check-tls.sh; (wget -q -nv --no-check-certificate -O - $url 2>/dev/null || curl -ksSL $url) | bash
rid=linux-x64
